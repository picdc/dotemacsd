...
- [[#utility][Utility]]
  ...
  - [[#ocaml-utilities][OCaml utilities]]
  - [[#region-0][Region]]
  - [[#repeat-mode-0][Repeat mode]]
  - [[#searching-must-read][Searching (must read!)]]
    - [[#consult][Consult]]
    - [[#isearch-must-read][Isearch (must read!)]]
    - [[#other-searching-tools][Other searching tools]]
  - [[#spell-and-grammar-checking][Spell and grammar checking]]
  - [[#which-key-0][Which key]]
  - [[#window-management-0][Window management]]
- [[#lsp-mode][LSP mode]]
...
The mark is often set with ~C-SPC~. It's used to store a position to which you can jump back (with ~C-U C-SPC~ for a local mark or ~C-x C-SPC~ for a global mark).

Marks are used to create regions with ~C-SPC~ and moving the cursor but can also be used to store a position with ~C-SPC C-SPC~.

** OCaml utilities
:PROPERTIES:
:CUSTOM_ID: ocaml-utilities
:END:

- It is recommended to start a ~dune watch~ process. This will greatly improve the LSP experience

  - You can start one with `ocaml-utils-compile`
    #+attr_org: :align left :width 800
       [[file:resources/images/ocaml-utils-compile.png]]
...
*** Isearch (must read!)
:PROPERTIES:
:CUSTOM_ID: isearch-must-read
:END:

Isearch is one of the most powerful tools in emacs to find things.
It is sadly often underused or misused so I'll try to give you some quick tips.

First of all, an isearch session can be started with

- ~C-s~ or ~C-r~, start writing some characters to start the search (will be preemptively filled if used with a region selected)

- ~M-s .~ start searching the symbol at point. There is a difference between an ordinary search and a symbol search. A symbol search will also consider the boundaries when searching. For example:

  #+begin_src emacs-lisp
  type data_dir = {data : string}
  ...
  - ~C-s data~ will highlight all the occurrences of the word ~data~ (~data_dir~, ~data~, ~my_data~ etc)

  - ~M-s .~ over ~data~ will only highlight the symbols ~data~ (~data : string~, ~data~, ~d.data~)

Also, don't quit your isearch session when you want to edit your search, do this instead:

- ~C-g~: Deletes the part of the current search that is not matching anything

- ~M-e~: Edits the current search

- ~M-c~ (or ~c~ once the input can't be edited): Case sensitivity

- ~M-_~: toggles the symbol mode

When starting an ~isearch~ session, the [[#mark][mark]] is set to where the cursor was before the session started. *You can easily jump back to where you were before searching* with ~C-u C-SPC~!

*** Other searching tools
:PROPERTIES:
:CUSTOM_ID: other-searching-tools
:END:
